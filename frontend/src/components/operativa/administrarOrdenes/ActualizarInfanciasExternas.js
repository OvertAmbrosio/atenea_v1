...
import { Button, Col, Row, Statistic, Upload } from 'antd';
...
export default function ActualizarInfanciasExternas() {
  ...
  async function guardarArchivo() {
    if (ordenesObtenidas.length === 0) {
      ...
      await postOrdenes({
        metodo: ordenes.SUBIR_INFANCIAS_EXTERNAS, 
        ordenesExternas: ordenesObtenidas 
      }).catch((err) => console.log(err)).finally(() => setLoadingButton(false));
  ...
  function cargarArchivo(file) {
    ...
    convertirExcel(file).then(async(objJson) => {
      ...
      await asignarValores(objJson, 'infancias').then((data) => {
