...
function ExcelAsistenciaTecnico({ data=[], dias=[], nombre }) {
  ...
  function guardarArchivo(component) {
    try {
      ...
      let indexHoraIniciado = 1
      rows.forEach((row) => {
        if (row.type === 'header') {
          console.log();
          indexEstado = row.cells.findIndex((e) => e.value === 'Estado');
          indexHoraIniciado = row.cells.findIndex((e) => e.value === 'Hora Ruta');
        ...
        if (row.type === 'data') {
          ...
          if (indexHoraIniciado > -1 ) {
            let valor = row.cells[indexHoraIniciado].value;
            console.log(row.cells[indexHoraIniciado]);
            row.cells[indexHoraIniciado].value = moment(valor).isValid ? moment(valor).format("HH:mm") : '-'
          }
          ...
          dias && dias.length > 0 ?
            dias.map((d, i) => (
              <ExcelExportColumnGroup 
                key={i}
                title={`${d}`} width={150}
                headerCellOptions={{...headerOptions, textAlign: "center"}}
              >
                <ExcelExportColumn
                  field={`${d}.estado`}
                  title={"Estado"} width={60}
                  headerCellOptions={headerOptions}
                  cellOptions={{ textAlign: 'center'}}
                />
                <ExcelExportColumn
                  field={`${d}.fecha_iniciado`}
                  title={"Hora Ruta"} width={60}
                  headerCellOptions={headerOptions}
                  cellOptions={{ textAlign: 'center'}}
                />
              </ExcelExportColumnGroup>
